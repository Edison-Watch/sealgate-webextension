...
- The Chrome DevTools MCP server is registered in `.mcp.json` as `chrome_devtools`; its
  tools appear as `mcp__chrome_devtools__*`. Use only those tools for browser work.
- Project permissions live in `.claude/settings.json`. The npm scripts below and the
  `chrome_devtools` tools are pre-approved; `dist/` is write-denied because it is
  generated output.
